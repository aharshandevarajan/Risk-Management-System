...
` : ''}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-3xl border border-slate-700/50 bg-gradient-to-br from-slate-900 to-slate-950 shadow-2xl">
        <div className={`relative overflow-hidden border-b border-slate-700/50 bg-gradient-to-r ${getSeverityColor()} p-6`}>
          <div className="relative z-10">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-white/80">
                  {risk.threatType} | {risk.affectedAsset}
                </p>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <span className="text-xs text-white/60">Risk Score: {risk.riskScore}</span>
              {cached && <span className="rounded-full bg-white/20 px-3 py-1 text-xs font-bold text-white">Cached</span>}
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-slate-700/50 bg-slate-900/50 p-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex gap-3">
              <button onClick={handleExportTXT} disabled={!insight} className="btn-secondary text-xs">
                Export
              </button>
              <button onClick={handleRegenerate} disabled={regenerating || loading} className="btn-primary text-xs">
